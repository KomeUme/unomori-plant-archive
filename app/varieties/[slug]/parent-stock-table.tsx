  ...
  return <>
    ...
    {selectedImage ? <div className="image-modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="image-modal-title" onPointerDown={(event) => { if (event.target === event.currentTarget) closeImage(); }}>
      <div className="image-modal-content">
        ...
        <div className={`image-modal-stage${imageZoom > 1 ? ' is-zoomed' : ''}${isImagePanning ? ' is-panning' : ''}`} onWheel={zoomImageWithWheel} onPointerDown={(event) => { if (event.target === event.currentTarget) { closeImage(); return; } startImagePan(event); }} onPointerMove={moveImagePan} onPointerUp={finishImagePan} onPointerCancel={cancelImagePan} onDoubleClick={resetImageView}>

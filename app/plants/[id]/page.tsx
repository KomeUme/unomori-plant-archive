...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!plant) return { title: '記録が見つかりません｜HAGUKUMU PLANTS' };
  return {
    title: `${plant.id}｜${plant.name}｜HAGUKUMU PLANTS`,
...
export default async function PlantDetailPage({ params }: PageProps) {
  ...
  if (!plant) notFound();
  return <main><Header />
    <section className="plant-detail"><a className="back-link" href="/records">← 成長記録一覧へ戻る</a><div className="detail-top"><img src={plant.image} alt={`${plant.name} ${plant.id}`} /><div><p className="eyebrow">PLANT RECORD</p><strong className="detail-id">{plant.id}</strong><h1>{plant.name}</h1><p className="detail-note">{plant.note}</p><dl><div><dt>分類</dt><dd>{plant.family}</dd></div><div><dt>親株</dt><dd>{plant.mother}</dd></div><div><dt>記録開始</dt><dd>{plant.started}</dd></div><div><dt>現在の状態</dt><dd>{plant.status}</dd></div></dl></div></div></section>

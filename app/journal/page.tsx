  ...
  return <main><Header />
    <section className="section journal-section page-section">
      ...
      <div className="journal-browse" aria-label="記事を絞り込む">
        {isVictoriaReginaeSelection ? <div className="journal-selection-breadcrumb" aria-label="現在の絞り込み条件"><p>選択中</p><div><a href={journalHref({ category: null, variety: null, managementFamily: null, management: null, page: null })}>すべての記事</a><span>/</span><a href={journalHref({ variety: null, managementFamily: null, management: null, page: null })}>アガベ</a><span>/</span><b>笹の雪</b></div></div> : <><div className="journal-filter-step"><p>分類</p><div className="journal-category-tabs">{categoryTabs.map((category) => <a key={category} className={activeCategory === category ? 'is-active' : ''} href={journalHref({ category: category === 'すべて' ? null : category, variety: null, managementFamily: null, management: null, page: null })}>{category}</a>)}</div></div>{showVarietyStep && <div className="journal-filter-step journal-subfilter"><p>品種を選ぶ</p><div className="journal-category-tabs journal-variety-tabs"><a className={!activeVariety ? 'is-active' : ''} href={journalHref({ variety: null, managementFamily: null, management: null, page: null })}>すべて</a>{featuredVarieties.map((variety) => <a key={variety} className={activeVariety === variety ? 'is-active' : ''} href={journalHref({ variety, managementFamily: null, management: null, page: null })}>{variety}</a>)}{otherVarieties.length > 0 && <a className={activeVariety === otherVarietyKey ? 'is-active' : ''} href={journalHref({ variety: otherVarietyKey, managementFamily: null, management: null, page: null })}>その他の品種</a>}</div></div>}</>}
        {isVictoriaReginaeSelection && <div className="journal-filter-step journal-management-family-filter"><p>系統を選ぶ</p><div className="journal-category-tabs journal-management-family-tabs">{managementFamilies.map((family) => <a key={family} className={activeManagementFamily === family ? 'is-active' : ''} href={journalHref({ managementFamily: family, management: null, page: null })}>{managementFamilyLabels[family] ?? `${family}系`}</a>)}</div></div>}

...
const guides = [
  ['01', '光', 'LIGHT', '株の状態を見ながら、少しずつ日差しに慣らします。急に強い光へ移すことは避けます。'],
  ['02', '風', 'AIR', '蒸れを避けるため、年間を通して緩やかな風を確保します。強風が続く場所では株を守ります。'],
  ['03', '水', 'WATER', '用土の乾きと根の状態を確認してから与えます。季節ごとの気温と生育に合わせて間隔を調整します。'],
  ['04', '土', 'SOIL', '水はけと通気性を基本に、根の状態と置き場所に合わせて配合を選びます。'],
];
...
const guideIntroduction = [
  ...
  'これは、長期間病気で入院していた人に、回復させたいからといって突然ステーキのような負担の大きな食事を与えるようなものです。植物も同様に、状態に合わせて少しずつ本来の環境へ戻していく必要があります。',
...
export default function CareGuidePage() {
  return <main><Header />
    ...
    <section className="section guide-section page-section"><div className="guide-content"><div className="guide-introduction"><p className="eyebrow">BEFORE YOU BEGIN</p><h2>まず、植物の今の状態を見ます。</h2>{guideIntroduction.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div><figure className="agave-annual-graph"><picture><source media="(max-width: 720px)" srcSet={siteHref('/agave-annual-graph-mobile.jpg')} /><img src={siteHref('/agave-annual-graph-desktop.jpg')} alt="アガベの年間育成グラフ。月ごとの育成状態、水やり量、植え替え適期を示す。" /></picture></figure><div className="guide-points-heading"><p className="eyebrow">CARE POINTS</p><h2>栽培におけるポイント</h2></div><div className="guide-grid">{guides.map(([no, title, en, body]) => <article key={no}><span className="guide-point-number">{no}</span><p className="eyebrow">{en}</p><h3>{title}</h3><p>{body}</p></article>)}</div><div className="season-note"><p className="eyebrow">THIS MONTH / AUGUST</p><b>夏の管理メモ</b><p>強い西日と葉焼けに注意。水やり後は株元に風を通し、夜間の蒸れを避けます。</p></div></div></section>
    <Footer />
  </main>;
}

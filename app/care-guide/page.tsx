...
import { CareGuideHashFocus } from './hash-focus';
...
function FirstCareLinks() {
  ...
  return <section className="first-care-links" aria-labelledby="first-care-title"><div className="first-care-links-heading"><p className="eyebrow">START HERE</p><h2 id="first-care-title">はじめに確認</h2><p>栽培を始めるうえでの基礎的なステップです。特に水やりは、習得までに3年かかるともいわれるほど奥深く、一度の判断が株に大きく影響することもあります。すべてはトライ＆エラーです。</p></div><div>{links.map(({ href, title, englishTitle }) => <a id={href.split('/').pop()} href={siteHref(href)} key={href}><div><p className="eyebrow">{englishTitle}</p><span>{title}</span></div><b aria-hidden="true">→</b></a>)}</div></section>;
}

export default function CareGuidePage() {
  return <main><CareGuideHashFocus /><Header />
    <section className="page-hero care-guide-page-hero"><p className="eyebrow">CARE GUIDE</p><h1>栽培ガイド</h1><p>サボテン、アガベ、夏型コーデックスを育ててきた実体験をもとに、栽培の要点をまとめています。関東圏の屋外・多肉棚での管理を前提とした内容のため、お住まいの地域や環境に合わせて参考程度にご活用ください。一個人の経験に基づくものなので、複数の情報源や生産者の意見も併せてご参照ください。</p><span className="care-guide-sasanoyuki-illustration"><img src={siteHref('/care-guide-sasanoyuki.png')} alt="" /></span></section>
    <section className="section guide-section page-section"><div className="guide-content"><div className="guide-introduction"><p className="eyebrow">BEFORE YOU BEGIN</p><h2>まず、植物の状態を確認。</h2>{guideIntroduction.map(({ title, paragraphs }) => <div className="guide-intro-block" key={title}><h3>{title}</h3>{paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div>)}</div><div className="guide-points-heading"><p className="eyebrow">CARE POINTS</p><h2>栽培におけるポイント</h2><p className="guide-priority">優先順位　光 <span>＞</span> 風 <span>＞</span> 水 <span>＞</span> 土</p></div><div className="guide-grid">{guides.map(({ icon, title, englishTitle, body, more }) => <article key={title}><span className="guide-point-icon"><GuidePointIcon type={icon} /></span><h3>{title}<small>{englishTitle}</small></h3><p>{body} {more}</p></article>)}</div><FirstCareLinks /><div className="guide-points-heading annual-guide-heading"><p className="eyebrow">ANNUAL CARE</p><h2>年間を通した育成方法</h2></div><AnnualCareChart /><div className="annual-chart-scope"><p><b>対象</b>　サボテン、アガベ、夏型コーデックスなど</p><p>地域・置き場所・雨の当たり方・株の状態によって変わるため、表は目安として使い、日々の状態を見ながら調整してください。</p></div><div className="annual-graph-note"><p className="eyebrow">ABOUT THE SEASONS</p><p>日本では四季によって、気温・日照・降水量・湿度が大きく変わります。こうした短期間の環境変化は、植物が原生地で経験する条件とは異なることがあります。栽培では日本の季節に合わせながら、原生地に近い光・風・水・温度のバランスを、株の状態に応じて整えることが大切です。</p></div><div className="seasonal-care seasonal-care-with-link"><div className="guide-points-heading seasonal-care-heading"><p className="eyebrow">SEASONAL CARE</p><h2>季節ごとの注意点</h2></div><div className="seasonal-care-table-wrap"><table className="seasonal-care-table"><thead><tr><th scope="col">項目</th>{['春', '夏', '秋', '冬'].map((season) => <th scope="col" key={season}>{season}</th>)}</tr></thead><tbody>{seasonalCare.map(({ item, values }) => <tr key={item}><th scope="row">{item}</th>{values.map((value, index) => <td key={`${item}-${index}`}>{value}</td>)}</tr>)}</tbody></table></div></div><div className="guide-points-heading troubleshooting-heading"><p className="eyebrow">TROUBLESHOOTING</p><h2>困ったときは</h2></div><a className="guide-method-link troubleshooting-link" id="troubleshooting" href={siteHref('/care-guide/diagnosis')}><div><p className="eyebrow">TROUBLESHOOTING</p><h2>よくあるトラブル Q&amp;A</h2></div><span>詳しく見る <b aria-hidden="true">→</b></span></a><div className="guide-points-heading growing-tips-heading"><p className="eyebrow">GROWING TIPS</p><h2>栽培のヒント</h2></div><a className="guide-method-link guide-method-link-secondary" id="soil" href={siteHref('/care-guide/soil')}><div><p className="eyebrow">SOIL</p><h2>用土の作り方</h2></div><span>詳しく見る <b aria-hidden="true">→</b></span></a><a className="guide-method-link guide-method-link-secondary" id="shading" href={siteHref('/care-guide/shading')}><div><p className="eyebrow">SHADING</p><h2>遮光の方法と考え方</h2></div><span>詳しく見る <b aria-hidden="true">→</b></span></a><a className="guide-method-link guide-method-link-secondary" id="winter-care" href={siteHref('/care-guide/winter-care')}><div><p className="eyebrow">WINTER CARE</p><h2>冬越し・霜対策</h2></div><span>詳しく見る <b aria-hidden="true">→</b></span></a><a className="guide-method-link guide-method-link-secondary shelf-guide-link" id="shelf" href={siteHref('/care-guide/shelf')}><div><p className="eyebrow">SHELF</p><h2>自作棚の作り方</h2><p className="guide-method-description">ホームセンターで揃う自作棚の作り方。</p></div><span>詳しく見る <b aria-hidden="true">→</b></span></a></div></section>
    <Footer />
  </main>;
}

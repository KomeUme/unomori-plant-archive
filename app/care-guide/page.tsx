...
export default function CareGuidePage() {
  return <main><Header />
    ...
    <section className="section guide-section page-section"><div className="guide-content"><div className="guide-introduction"><p className="eyebrow">BEFORE YOU BEGIN</p><h2>まず、植物の状態を確認。</h2>{guideIntroduction.map(({ title, paragraphs }) => <div className="guide-intro-block" key={title}><h3>{title}</h3>{paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div>)}</div><div className="guide-points-heading"><p className="eyebrow">CARE POINTS</p><h2>栽培におけるポイント</h2><p className="guide-priority">優先順位　光 <span>＞</span> 風 <span>＞</span> 水 <span>＞</span> 土</p></div><div className="guide-grid">{guides.map(({ icon, title, englishTitle, body, more }) => <article key={title}><span className="guide-point-icon"><GuidePointIcon type={icon} /></span><h3>{title}<small>{englishTitle}</small></h3><p>{body} {more}</p></article>)}</div><a className="guide-method-link guide-method-link-primary" href={siteHref('/care-guide/watering')}><div><p className="guide-method-label">はじめに確認</p><h2>水やりの方法</h2></div><span>詳しく見る <b aria-hidden="true">→</b></span></a><a className="guide-method-link guide-method-link-primary" href={siteHref('/care-guide/repotting')}><div><p className="guide-method-label">はじめに確認</p><h2>植え替え直後の管理</h2></div><span>詳しく見る <b aria-hidden="true">→</b></span></a><a className="guide-method-link guide-method-link-primary" href={siteHref('/care-guide/acclimation')}><div><p className="guide-method-label">はじめに確認</p><h2>購入直後の管理</h2></div><span>詳しく見る <b aria-hidden="true">→</b></span></a><a className="guide-method-link guide-method-link-secondary" href={siteHref('/care-guide/soil')}><div><p className="eyebrow">SOIL</p><h2>用土の作り方</h2></div><span>詳しく見る <b aria-hidden="true">→</b></span></a><div className="guide-points-heading annual-guide-heading"><p className="eyebrow">ANNUAL CARE</p><h2>年間を通した育成方法</h2></div><AnnualCareChart /><div className="annual-chart-scope"><p><b>対象</b>　サボテン、アガベ、夏型コーデックスなど</p><p>地域・置き場所・雨の当たり方・株の状態によって変わるため、表は目安として使い、日々の状態を見ながら調整してください。</p></div><div className="annual-graph-note"><p className="eyebrow">ABOUT THE SEASONS</p><p>日本では四季によって、気温・日照・降水量・湿度が大きく変わります。こうした短期間の環境変化は、植物が原生地で経験する条件とは異なることがあります。栽培では日本の季節に合わせながら、原生地に近い光・風・水・温度のバランスを、株の状態に応じて整えることが大切です。</p></div><div className="seasonal-care seasonal-care-with-link"><div className="guide-points-heading seasonal-care-heading"><p className="eyebrow">SEASONAL CARE</p><h2>季節ごとの注意点</h2></div><div className="seasonal-care-table-wrap"><table className="seasonal-care-table"><thead><tr><th scope="col">項目</th>{['春', '夏', '秋', '冬'].map((season) => <th scope="col" key={season}>{season}</th>)}</tr></thead><tbody>{seasonalCare.map(({ item, values }) => <tr key={item}><th scope="row">{item}</th>{values.map((value, index) => <td key={`${item}-${index}`}>{value}</td>)}</tr>)}</tbody></table></div></div><a className="guide-method-link guide-method-link-secondary" href={siteHref('/care-guide/diagnosis')}><div><p className="eyebrow">TROUBLESHOOTING</p><h2>不調診断</h2></div><span>詳しく見る <b aria-hidden="true">→</b></span></a><a className="guide-method-link guide-method-link-secondary" href={siteHref('/care-guide/winter-care')}><div><p className="eyebrow">WINTER CARE</p><h2>冬越し・雨対策</h2></div><span>詳しく見る <b aria-hidden="true">→</b></span></a><a className="guide-method-link guide-method-link-secondary shelf-guide-link" href={siteHref('/care-guide/shelf')}><div><p className="eyebrow">SHELF</p><h2>自作棚の作り方</h2><p className="guide-method-description">ホームセンターで揃う自作棚の作り方。</p></div><span>詳しく見る <b aria-hidden="true">→</b></span></a></div></section>
    <Footer />
  </main>;
}

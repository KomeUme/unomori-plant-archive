  ...
  return <main><Header />
    <section className="stock-detail-top">
      ...
      <div className="stock-detail-card">
        <div className="stock-detail-image">{stock.image ? <ParentStockDetailImage image={stock.image} displayName={stock.managementName ?? stock.acquisitionName} stockId={stock.id} /> : <span>PHOTO<br />PENDING</span>}</div>
        <div className="stock-detail-copy"><p className="eyebrow">PARENT STOCK / {variety.name}</p><strong>{stock.id}</strong>{(stock.managementName || stock.acquisitionName) && <div className="stock-name-heading"><span className="stock-name-label">{stock.managementName ? '管理名' : '入手時名'}</span><h1>{stock.managementName ?? stock.acquisitionName}</h1></div>}<div className="stock-provenance">{stock.managementName && stock.acquisitionName && <p>入手時名：{stock.acquisitionName}</p>}<p>入手元：{stock.acquiredFrom ?? '未記録'}</p><p>来歴：{stock.history ?? '未記録'}</p><p>管理開始日：{stock.managementStartedOn ?? '未記録'}</p></div><p>{stock.selectionReason}</p><dl><div><dt>学名</dt><dd>{variety.botanicalName}</dd></div><div><dt>備考</dt><dd>{stock.notes}</dd></div></dl></div>
      </div>
    </section>

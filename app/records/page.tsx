...
export default function RecordsPage() {
  ...
  useEffect(() => {
    // Query parameters are only available in the browser for a static export.
  ...
  return <main><Header />
    ...
    <section className="section records-section page-section">
      ...
      <div className="record-grid">{results.map((plant) => <a className="plant-card" key={plant.id} href={siteHref(`/plants/${plant.id}`)}><div className="card-image"><img src={plant.image} alt={`${plant.name} ${plant.id}`} /><span className={`status ${plant.status === '養生中' ? 'resting' : ''}`}>{plant.status}</span><span className="view-circle">↗</span></div><div className="card-body"><div className="id-row"><strong>{plant.id}</strong><span>{plant.family}</span></div><h3>{plant.name}</h3><p>{plant.note}</p><div className="card-meta"><span>記録開始 {plant.started}</span><span>親株 {plant.mother}</span></div><span className="text-link">成長履歴を見る <b>→</b></span></div></a>)}</div>
